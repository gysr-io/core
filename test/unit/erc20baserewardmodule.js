...
describe('ERC20BaseRewardModule', function () {
  ...
  let org, owner, controller, bob, alice, factory, treasury;
  before(async function () {
    [org, owner, controller, bob, alice, factory, treasury] = await web3.eth.getAccounts();
  });
  ...
  describe('funding', function () {
    ...
    describe('when funded for future start', function () {
      ...
      it('should not increase unlocked rewards immediately', async function () {
        ...
        await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
    ...
    describe('when number of active fundings is at max', function () {
      ...
      it('gas cost of update() should be under 1M', async function () {
        const res = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
      ...
      it('gas cost of unstake() should be under 1M', async function () {
        ...
        const res = await this.module.unstake(alice, alice, alice, shares(100), [], { from: owner });
        expect(res.receipt.gasUsed).is.lessThan(10 ** 6);
      });
    });
    ...
    describe('when number of fundings is at max but one schedule expires', function () {
      ...
      it('should have expected amount of funding unlocked', async function () {
        ...
        // 16 schedules, avg ~8 seconds off
        expect(await this.module.totalUnlocked()).to.be.bignumber.closeTo(tokens(850), tokens(8 * 16 * 100).div(days(90)));
      });
      ...
      it('should have expected amount of funding locked', async function () {
        ...
        // 1/2 * 15 * 100 + 200
        // 16 schedules, avg ~8 seconds off
        expect(await this.module.totalLocked()).to.be.bignumber.closeTo(tokens(950), tokens(8 * 16 * 100).div(days(90)));
      });

    });


    describe('when funded with protocol fee enabled', function () {
      ...
      it('should increase total locked minus fee', async function () {
        expect(await this.module.totalLocked()).to.be.bignumber.equal(tokens(997));
      });

      it('should increase treasury balance by fee amount', async function () {
        expect(await this.token.balanceOf(treasury)).to.be.bignumber.equal(tokens(3));
      });
      ...
      it('should emit RewardsFunded with reduced shares', async function () {
        expectEvent(
          this.res,
          'RewardsFunded',
    ...
    describe('when funded with protocol fee receiver unset', function () {
      ...
      beforeEach(async function () {
        ...
        // leave receiver at zero address
  ...
  describe('unlocking', function () {
    ...
    describe('with a single 90-day funding schedule', function () {
      ...
      it('should unlock in increments over time', async function () {
        // 50%
        await time.increaseTo(this.t0.add(days(45)));
        const res0 = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
        expect(await this.module.totalUnlocked()).to.be.bignumber.closeTo(tokens(500), TOKEN_DELTA);

        // 90%
        await time.increaseTo(this.t0.add(days(81)));
        const res1 = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
        expect(await this.module.totalUnlocked()).to.be.bignumber.closeTo(tokens(900), TOKEN_DELTA);

        // 100%
        await time.increaseTo(this.t0.add(days(90)));
        const res2 = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
        expect(await this.module.totalUnlocked()).to.be.bignumber.closeTo(tokens(1000), TOKEN_DELTA);
    ...
    describe('when funding period is 0 seconds', function () {
      beforeEach(async function () {
        ...
        this.res = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
      });
  ...
  describe('elastic reward token', function () {
    ...
    describe('when supply expands', function () {
      ...
      beforeEach(async function () {
        // advance 45 days
        await time.increaseTo(this.t0.add(days(45)));
        this.res0 = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });
        ...
        await time.increaseTo(this.t0.add(days(90)));
        this.res1 = await this.module.update(constants.ZERO_BYTES32, constants.ZERO_ADDRESS, [], { from: owner });

        // alice unstakes 25 tokens with 2x time multiplier
        // portion: (2.0 * 25) / (100 - 25 + 2.0 * 25) = 0.4
        this.res2 = await this.module.unstake(alice, alice, alice, shares(25), [], { from: owner });
      });
      ...
      it('should unlock shares at linear rate before and after expansion', async function () {
        expect(await this.module.lockedShares(this.elastic.address)).to.be.bignumber.closeTo(shares(500), SHARE_DELTA);
      });
